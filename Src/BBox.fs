...
type BBox =
    ...
    /// <returns>A ResizeArray of 1 to 6 points, or an empty ResizeArray if the plane does not intersect this bounding box.</returns>
    member b.IntersectPlane (pl:NPlane) : ResizeArray<Pnt> =
        ...
        if pts.Count > 2 then
            ...
            if az >= ax && az >= ay then // drop the Z-axis, sorting counter-clockwise around the world Z-axis
                ...
                if nz < 0.0 then pts.Reverse() // flip to be counter-clockwise around the plane normal
    ...
    static member inline intersectPlane (pl:NPlane) (b:BBox) : ResizeArray<Pnt> =
        b.IntersectPlane pl
