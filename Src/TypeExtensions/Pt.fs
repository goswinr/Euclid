...
module AutoOpenPt =
    ...
    type Pt with
        ...
        member inline p.DirectionDiamondTo(o:Pt) =
            ...
            let x = o.X-p.X
            let y = o.Y-p.Y
            if abs(x) < zeroLengthTolerance && abs(y) < zeroLengthTolerance then p.FailedDirectionDiamondTo o // don't compose error msg directly here to keep inlined code small.
            if y >= 0.0 then
                if x >= 0.0 then
                    y/(x+y)
                else
