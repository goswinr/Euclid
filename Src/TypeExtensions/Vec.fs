...
module AutoOpenVec =
    ...
    type Vec with
        ...
        member inline v.DirectionDiamondInXY =
            ...
            if abs(v.X) < zeroLengthTolerance && abs(v.Y) < zeroLengthTolerance then v.FailedDirectionDiamondInXY()                
            if v.Y >= 0.0 then
                if v.X >= 0.0 then
                    v.Y/(v.X+v.Y)
                else
        ...
        member inline v.Direction2PiInXY =
            // https://stackoverflow.com/a/14675998/969070
            if abs(v.X) < zeroLengthTolerance && abs(v.Y) < zeroLengthTolerance then v.FailedDirection2PiInXY()                
        ...
        member inline v.DirectionPiInXY =
            // https://stackoverflow.com/a/14675998/969070
            if abs(v.X) < zeroLengthTolerance && abs(v.Y) < zeroLengthTolerance then v.FailedDirectionPiInXY()                
            let a = Math.Atan2(v.Y, v.X)
            if a < 0. then
                a + Math.PI
            else
        ...
        member inline v.IsXAligned =
            ...
            let y = abs (v.Y)
            let z = abs (v.Z)
            if x+y+z < 1e-6 then EuclidException.Throw1 "Euclid.Vec.IsXAligned cannot not check very tiny vector. (tolerance 1e-6)" v
            else y < 1e-9 && z < 1e-9     
        ...
        static member inline isAngleLessThan1Degree(a:Vec, b:Vec) =
            let sa = a.LengthSq
            if sa < zeroLengthTolSquared then EuclidException.Raise "Euclid.Vec.isAngleLessThan1Degree: Vec a is too short: %s. Vec b:%s " a.AsString b.AsString
            let sb = b.LengthSq
            if sb < zeroLengthTolSquared then EuclidException.Raise "Euclid.Vec.isAngleLessThan1Degree: Vec b is too short: %s. Vec a:%s " b.AsString a.AsString
        ...
        static member inline isAngleLessThanQuatreDegree(a:Vec, b:Vec) =
            let sa = a.LengthSq
            if sa < zeroLengthTolSquared then EuclidException.Raise "Euclid.Vec.isAngleLessThanQuatreDegree: Vec a is too short: %s. Vec b:%s " a.AsString b.AsString
            let sb = b.LengthSq
            if sb < zeroLengthTolSquared then EuclidException.Raise "Euclid.Vec.isAngleLessThanQuatreDegree: Vec b is too short: %s. Vec a:%s " b.AsString a.AsString
            let au = a * (1.0 / sqrt sa )
            let bu = b * (1.0 / sqrt sb )
