...
import { join, concat } from "../fable_modules/fable-library-js.5.0.0/String.js";
...
import { failEmptySeq, failNull, failTooFewPoly3D, fail } from "./EuclidErrors.js";
...
import { offsetVariableWithDirections, getSegmentUnitVectors, getOffsetDirections, offsetConstantWithDirections } from "./Offset3D.js";
...
export function Polyline3D__get_AsFSharpCode(p) {
    return concat("Polyline3D.create [| ", join("; ", ResizeArr_map(Pnt__get_AsFSharpCode, p.points)), ..." |]");
}
...
export function Polyline3D__CloseInPlace_5E38073B(p, toleranceForAddingPoint) {
    ...
    if (((v_1 = ((a = Polyline3D__get_Start(p), (b = Polyline3D__get_End(p), Vec_$ctor_Z7AD9E565(a.X - b.X, a.Y - b.Y, a.Z - b.Z)))), ((v_1.X * v_1.X) + (v_1.Y * v_1.Y)) + (v_1.Z * v_1.Z))) < (toleranceForAddingPoint * toleranceForAddingPoint)) {
        const this$ = p.points;
...
export function Polyline3D__EvaluateAt_5E38073B(pl, t) {
    const i = ~~t | 0;
    const p = t - i;
    const countF = Polyline3D__get_Points(pl).length;
    if (t < 1E-06) {
        if (t < -1E-06) {
            fail(`Polyline3D.EvaluateAt: Parameter ${t} is less than 0.0`);
        }
        return item(0, Polyline3D__get_Points(pl));
    }
...
export function Polyline3D__ClosestParameter_Z394ECE4D(pl, p) {
    ...
    for (let i = 1; i <= (pts.length - 1); i++) {
        ...
        if (((dx !== 0) ? true : (dy !== 0)) ? true : (dz !== 0)) {
            const t$0027 = max(0, min(1, ((((p.X - a.X) * dx) + ((p.Y - a.Y) * dy)) + ((p.Z - a.Z) * dz)) / (((dx * dx) + (dy * dy)) + (dz * dz))));
            const dpx = p.X - (a.X + (dx * t$0027));
            const dpy = p.Y - (a.Y + (dy * t$0027));
            const dpz = p.Z - (a.Z + (dz * t$0027));
...
export function Polyline3D_transform(m, pl) {
    return Polyline3D_map((p) => {
        ...
        const y = p_3.Y;
        const z = p_3.Z;
        const sc = 1 / ((((m_4.M14 * x) + (m_4.M24 * y)) + (m_4.M34 * z)) + m_4.M44);
        return Pnt_$ctor_Z7AD9E565(((((m_4.M11 * x) + (m_4.M21 * y)) + (m_4.M31 * z)) + m_4.X41) * sc, ((((m_4.M12 * x) + (m_4.M22 * y)) + (m_4.M32 * z)) + m_4.Y42) * sc, ((((m_4.M13 * x) + (m_4.M23 * y)) + (m_4.M33 * z)) + m_4.Z43) * sc);
    }, pl);
}
...
export function Polyline3D_close_Z5A89AE96(pl) {
    ...
    if (((a_1 = item(0, points), (b_1 = ((this$_1 = points, item(this$_1.length - 1, this$_1))), (x = (a_1.X - b_1.X), (y = (a_1.Y - b_1.Y), (z = (a_1.Z - b_1.Z), ((x * x) + (y * y)) + (z * z))))))) < 1E-12) {
        setItem(Polyline3D__get_Points(np), Polyline3D__get_Points(np).length - 1, item(0, Polyline3D__get_Points(np)));
    }
    else {
        void (Polyline3D__get_Points(np).push(item(0, Polyline3D__get_Points(np))));
    }
    return np;
}
    ...
    if (pts.length < 2) {
        ...
        for (let idx = i + 1; idx <= lastIdx; idx++) {
            ...
            if (len_1 > distanceTolerance) {
                ...
                if (((a_4 = vPrev, (b_4 = vNext, ((a_4.X * b_4.X) + (a_4.Y * b_4.Y)) + (a_4.Z * b_4.Z)))) < angleTolerance) {
        ...
        if (Polyline3D__IsAlmostClosed_5E38073B(pl, distanceTolerance)) {
            if (((a_5 = vPrev, (b_5 = firstVec, ((a_5.X * b_5.X) + (a_5.Y * b_5.Y)) + (a_5.Z * b_5.Z)))) < angleTolerance) {
...
export function Polyline3D_offsetWithRef_39D084EF(polyLine, inPlaneOffsetDistance, perpendicularOffsetDistance, refNormal, loop = false) {
    let a_1, b_1, this$_1, x, y, z, this$_2, i, v;
    ...
    const isOpen = ((a_1 = item(0, pts), (b_1 = ((this$_1 = pts, item(this$_1.length - 1, this$_1))), (x = (a_1.X - b_1.X), (y = (a_1.Y - b_1.Y), (z = (a_1.Z - b_1.Z), ((x * x) + (y * y)) + (z * z))))))) > 1E-12;
    if (loop && isOpen) {
        const closedPts = ResizeArr_closeLoop(pts);
        const res = offsetConstantWithDirections(closedPts, getOffsetDirections(getSegmentUnitVectors(closedPts), refNormal, false, 0.9990482215818578, -0.9998476951563913), inPlaneOffsetDistance, perpendicularOffsetDistance);
        ...
        return Polyline3D_createDirectlyUnsafe_516DFD0A(offsetConstantWithDirections(pts, getOffsetDirections(getSegmentUnitVectors(pts), refNormal, isOpen, 0.9990482215818578, -0.9998476951563913), inPlaneOffsetDistance, perpendicularOffsetDistance));
    }
}
...
export function Polyline3D_offsetVarWithRef_3346ED26(polyLine, inPlaneOffsetDistance, perpendicularOffsetDistances, refNormal, loop = false, varDistParallelBehaviour = 1, considerColinearBelow = 0.9990482215818578, failAtUTurnAbove = -0.9961946980917455) {
    ...
    if (loop && isOpen) {
        ...
        const res = offsetVariableWithDirections(closedPts, uvs, getOffsetDirections(uvs, refNormal, false, considerColinearBelow, failAtUTurnAbove), inPlaneOffsetDistance, perpendicularOffsetDistances, true, varDistParallelBehaviour);
        (this$_2 = res, (i = ((this$_2.length - 1) | 0), (v = item(i, this$_2), (this$_2.splice(i, 1), v))));
        return Polyline3D_createDirectlyUnsafe_516DFD0A(res);
    }
    else {
        const uvs_1 = getSegmentUnitVectors(pts);
        return Polyline3D_createDirectlyUnsafe_516DFD0A(offsetVariableWithDirections(pts, uvs_1, getOffsetDirections(uvs_1, refNormal, isOpen, considerColinearBelow, failAtUTurnAbove), inPlaneOffsetDistance, perpendicularOffsetDistances, isOpen, varDistParallelBehaviour));
    }
}
