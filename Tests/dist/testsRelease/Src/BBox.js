...
export class BBox extends Record {
    ...
    toString() {
        const b = this;
        return `Euclid.BBox: Size: x=${float(b.MaxX - b.MinX)}|y=${float(b.MaxY - b.MinY)}|z=${float(b.MaxZ - b.MinZ)} (at X=${float(b.MinX)}|Y=${float(b.MinY)}|Z=${float(b.MinZ)})`;
    }
}
...
export function BBox__get_AsString(b) {
    return `Size: x=${float(b.MaxX - b.MinX)}|y=${float(b.MaxY - b.MinY)}|z=${float(b.MaxZ - b.MinZ)} (at X=${float(b.MinX)}|Y=${float(b.MinY)}|Z=${float(b.MinZ)})`;
}
...
export function BBox__IsTouching_668B4595(b, a, tol = 1E-06) {
    ...
    if (((xOverlap && yOverlap) && ((Math.abs(a.MinZ - b.MaxZ) <= tol) ? true : (Math.abs(b.MinZ - a.MaxZ) <= tol))) ? true : ((xOverlap && yTouch) && zOverlap)) {
        return true;
    }
...
export function BBox_expandRel(factor, b) {
    ...
    let center_1;
    const b_1 = b;
    center_1 = Pnt_$ctor_Z7AD9E565((b_1.MaxX + b_1.MinX) * 0.5, (b_1.MaxY + b_1.MinY) * 0.5, (b_1.MaxZ + b_1.MinZ) * 0.5);
    const sizeX_1 = ((b_2 = b, b_2.MaxX - b_2.MinX)) * factor;
    const sizeY_1 = ((b_3 = b, b_3.MaxY - b_3.MinY)) * factor;
    const sizeZ_1 = ((b_4 = b, b_4.MaxZ - b_4.MinZ)) * factor;
    if (!(sizeX_1 >= 0)) {
    ...
    return BBox_$ctor_76A78260(center_1.X - (sizeX_1 * 0.5), center_1.Y - (sizeY_1 * 0.5), center_1.Z - (sizeZ_1 * 0.5), center_1.X + (sizeX_1 * 0.5), center_1.Y + (sizeY_1 * 0.5), center_1.Z + (sizeZ_1 * 0.5));
}
...
export function BBox_expandRelXYZ(factorX, factorY, factorZ, b) {
    ...
    const b_1 = b;
    center_1 = Pnt_$ctor_Z7AD9E565((b_1.MaxX + b_1.MinX) * 0.5, (b_1.MaxY + b_1.MinY) * 0.5, (b_1.MaxZ + b_1.MinZ) * 0.5);
    const sizeX_1 = ((b_2 = b, b_2.MaxX - b_2.MinX)) * factorX;
    const sizeY_1 = ((b_3 = b, b_3.MaxY - b_3.MinY)) * factorY;
    const sizeZ_1 = ((b_4 = b, b_4.MaxZ - b_4.MinZ)) * factorZ;
    if (!(sizeX_1 >= 0)) {
        fail(`BBox.createFromCenter sizeX is negative: ${sizeX_1}, sizeY is: ${sizeY_1}, sizeZ is: ${sizeZ_1}, center: ${Pnt__get_AsString(center_1)}`);
    }
    if (!(sizeY_1 >= 0)) {
