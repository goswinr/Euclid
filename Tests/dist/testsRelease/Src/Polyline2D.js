...
import { printf, toFail, join, concat } from "../fable_modules/fable-library-js.5.0.0/String.js";
...
import { failEmptySeq, failNull, failTooFewPoly2D, fail } from "./EuclidErrors.js";
...
import { CellHeap__Pop, CellHeap__get_Count, Cell, CellHeap__Add_3F6CBBFD, CellHeap_$ctor } from "./PolyLabel.js";
...
import { offsetVariableWithDirections, makeOffsetDirections, offsetWithDirections } from "./Offset2D.js";
...
export function Polyline2D__get_AsFSharpCode(p) {
    return concat("Polyline2D.create [| ", join("; ", ResizeArr_map(Pt__get_AsFSharpCode, p.points)), ..." |]");
}
...
export function Polyline2D__CloseInPlace_5E38073B(p, toleranceForAddingPoint) {
    ...
    if (((v_1 = ((a = Polyline2D__get_Start(p), (b = Polyline2D__get_End(p), Vc_$ctor_7B00E9A0(a.X - b.X, a.Y - b.Y)))), (x = v_1.X, (y = v_1.Y, (x * x) + (y * y))))) < (toleranceForAddingPoint * toleranceForAddingPoint)) {
        const this$ = p.points;
...
export function Polyline2D__EvaluateAt_5E38073B(pl, t) {
    const i = ~~t | 0;
    const p = t - i;
    const countF = Polyline2D__get_Points(pl).length;
    if (t < 1E-06) {
        if (t < -1E-06) {
            fail(`Polyline2D.EvaluateAt: Parameter ${t} is less than 0.0`);
        }
        return item(0, Polyline2D__get_Points(pl));
    }
...
export function Polyline2D__ClosestParameter_6ADE94FD(pl, p) {
    ...
    for (let i = 1; i <= (pts.length - 1); i++) {
        ...
        const dy = b_2.Y - a.Y;
        if ((dx !== 0) ? true : (dy !== 0)) {
            const t$0027 = max(0, min(1, (((p.X - a.X) * dx) + ((p.Y - a.Y) * dy)) / ((dx * dx) + (dy * dy))));
            const dpx = p.X - (a.X + (dx * t$0027));
            const dpy = p.Y - (a.Y + (dy * t$0027));
...
export function Polyline2D__WindingNumber_6ADE94FD(pl, point) {
    ...
    if (pts.length > 0) {
        let this$ = item(0, pts);
        for (let i = 1; i <= (pts.length - 1); i++) {
            let aa, bb, det, aa_1, bb_1, det_1;
            const next = item(i, pts);
            if (this$.Y <= py) {
                if ((next.Y > py) && (((aa = this$, (bb = next, (det = (((px - aa.X) * (bb.Y - aa.Y)) - ((py - aa.Y) * (bb.X - aa.X))), (det > 0) ? 1 : ((det < 0) ? -1 : 0))))) > 0)) {
                    winding = ((winding - 1) | 0);
                }
            }
            else if ((next.Y <= py) && (((aa_1 = this$, (bb_1 = next, (det_1 = (((px - aa_1.X) * (bb_1.Y - aa_1.Y)) - ((py - aa_1.Y) * (bb_1.X - aa_1.X))), (det_1 > 0) ? 1 : ((det_1 < 0) ? -1 : 0))))) < 0)) {
                winding = ((winding + 1) | 0);
            }
            this$ = next;
        }
...
export function Polyline2D__Contains_6ADE94FD(p, pt) {
    ...
    if (pts.length < 3) {
        ...
        for (let i = 1; i <= (pts.length - 1); i++) {
            const pj = item(i, pts);
            if (((pi.Y > y) !== (pj.Y > y)) && (pt.X < ((((pj.X - pi.X) * (y - pi.Y)) / (pj.Y - pi.Y)) + pi.X))) {
                inside = !inside;
            }
            pi = pj;
        }
...
export function Polyline2D__FindLablePoint_5E38073B(pl, precision) {
    ...
    if ((width < precision) ? true : (height < precision)) {
        ...
        return [c, Polyline2D__SignedDistanceTo_6ADE94FD(pl, c)];
    }
    else {
        ...
        while (y < maxY) {
            let x = minX;
            while (x < maxX) {
                let point, h_1, distance;
                CellHeap__Add_3F6CBBFD(heap, (point = Pt_$ctor_7B00E9A0(x + halfCell, y + halfCell), (h_1 = halfCell, (distance = Polyline2D__SignedDistanceTo_6ADE94FD(pl, point), new Cell(point.X, point.Y, h_1, distance, distance + (h_1 * 1.4142135623730951))))));
                x = (x + cellSize);
            }
            y = (y + cellSize);
        }
        ...
        bestCell = (new Cell(point_1.X, point_1.Y, 0, distance_1, distance_1 + (0 * 1.4142135623730951)));
        let bboxCell;
        let point_2;
        const r = bRect;
        ...
        bboxCell = (new Cell(point_2.X, point_2.Y, 0, distance_2, distance_2 + (0 * 1.4142135623730951)));
        ...
        while (CellHeap__get_Count(heap) > 0) {
            ...
            if ((cell_1.MaxDistance - bestCell.Distance) <= precision) {
                ...
                c_1 = (new Cell(point_3.X, point_3.Y, h_6, distance_3, distance_3 + (h_6 * 1.4142135623730951)));
                ...
                c_2 = (new Cell(point_4.X, point_4.Y, h_8, distance_4, distance_4 + (h_8 * 1.4142135623730951)));
                ...
                c_3 = (new Cell(point_5.X, point_5.Y, h_10, distance_5, distance_5 + (h_10 * 1.4142135623730951)));
                ...
                c_4 = (new Cell(point_6.X, point_6.Y, h_12, distance_6, distance_6 + (h_12 * 1.4142135623730951)));
...
export function Polyline2D_close_Z5A89AEF5(pl) {
    ...
    if (((a_1 = item(0, ps), (b_1 = ((this$_1 = ps, item(this$_1.length - 1, this$_1))), (vx = (a_1.X - b_1.X), (vy = (a_1.Y - b_1.Y), (vx * vx) + (vy * vy)))))) < 1E-12) {
        setItem(Polyline2D__get_Points(np), Polyline2D__get_Points(np).length - 1, item(0, Polyline2D__get_Points(np)));
    }
    else {
        void (Polyline2D__get_Points(np).push(item(0, Polyline2D__get_Points(np))));
    }
    return np;
}
...
export function Polyline2D_closeInPlace_Z5A89AEF5(pl) {
    ...
    if (((a_1 = item(0, points), (b_1 = ((this$_1 = points, item(this$_1.length - 1, this$_1))), (vx = (a_1.X - b_1.X), (vy = (a_1.Y - b_1.Y), (vx * vx) + (vy * vy)))))) < 1E-12) {
        setItem(points, points.length - 1, item(0, points));
    }
    else {
        void (points.push(item(0, points)));
    }
}
    ...
    if (pts.length < 2) {
        ...
        for (let idx = i + 1; idx <= lastIdx; idx++) {
            ...
            if (len_1 > distanceTolerance) {
                ...
                if (((a_4 = vPrev, (b_4 = vNext, (a_4.X * b_4.X) + (a_4.Y * b_4.Y)))) < angleTolerance) {
        ...
        if (Polyline2D__IsAlmostClosed_5E38073B(pl, distanceTolerance)) {
            if (((a_5 = vPrev, (b_5 = firstVec, (a_5.X * b_5.X) + (a_5.Y * b_5.Y)))) < angleTolerance) {
                void (nps.push((this$_2 = pts, item(this$_2.length - 1, this$_2))));
            }
            else {
                setItem(nps, 0, (this$_3 = nps, item(this$_3.length - 1, this$_3)));
            }
        }
...
export function Polyline2D_offset_Z45C468A5(polyLine, constantOffsetDistance, loop = false, checkOrientation = true, uTurnBehavior = 1, useUTurnBehaviorAbove = -0.9961946980917455) {
    let a_1, b_1, this$_1, vx, vy, this$_2, i, v;
    ...
    if (loop && (((a_1 = item(0, pts), (b_1 = ((this$_1 = pts, item(this$_1.length - 1, this$_1))), (vx = (a_1.X - b_1.X), (vy = (a_1.Y - b_1.Y), (vx * vx) + (vy * vy)))))) > 1E-12)) {
        const closedPts = ResizeArr_closeLoop(pts);
        const res = offsetWithDirections(closedPts, makeOffsetDirections(closedPts), constantOffsetDistance_1, uTurnBehavior, useUTurnBehaviorAbove);
        (this$_2 = res, (i = ((this$_2.length - 1) | 0), (v = item(i, this$_2), (this$_2.splice(i, 1), v))));
        return Polyline2D_createDirectlyUnsafe_Z5FD8CF3C(res);
    }
    else {
        return Polyline2D_createDirectlyUnsafe_Z5FD8CF3C(offsetWithDirections(pts, makeOffsetDirections(pts), constantOffsetDistance_1, uTurnBehavior, useUTurnBehaviorAbove));
    }
}
...
export function Polyline2D_offsetVar_3B75EE3F(polyLine, multipleOffsetDistances, loop = false, checkOrientation = true, varDistParallelBehavior = 1, uTurnBehavior = 1, useVarDistParallelBehaviorBelow = 0.9961946980917455, useUTurnBehaviorAbove = -0.9961946980917455) {
    let a_1, b_1, this$_1, vx, vy, this$_2, i_1, v;
    const pts = Polyline2D__get_Points(polyLine);
    if (pts.length < 2) {
        fail(`Polyline2D.offset: Polyline2D must have at least 2 points but has ${pts.length} points. ${polyLine}`);
    }
    ...
    if (loop && (((a_1 = item(0, pts), (b_1 = ((this$_1 = pts, item(this$_1.length - 1, this$_1))), (vx = (a_1.X - b_1.X), (vy = (a_1.Y - b_1.Y), (vx * vx) + (vy * vy)))))) > 1E-12)) {
        ...
        const closedPts = ResizeArr_closeLoop(pts);
        const res = offsetVariableWithDirections(closedPts, makeOffsetDirections(closedPts), distances, varDistParallelBehavior, uTurnBehavior, useVarDistParallelBehaviorBelow, useUTurnBehaviorAbove);
        (this$_2 = res, (i_1 = ((this$_2.length - 1) | 0), (v = item(i_1, this$_2), (this$_2.splice(i_1, 1), v))));
        return Polyline2D_$ctor_Z5FD8CF3C(res);
    }
    else {
        return Polyline2D_$ctor_Z5FD8CF3C(offsetVariableWithDirections(pts, makeOffsetDirections(pts), distances, varDistParallelBehavior, uTurnBehavior, useVarDistParallelBehaviorBelow, useUTurnBehaviorAbove));
    }
}
...
export function Polyline2D_tryFindSelfIntersection_Z5A89AEF5(pl) {
    ...
    if (Polyline2D__get_IsClosed(pl)) {
        ...
        let matchValue_1;
        const pA_1 = item(0, pts);
        let pB_1;
        const this$_3 = pts;
        pB_1 = item(this$_3.length - 2, this$_3);
        const vA_1 = item(0, segmentVs);
        let vB_1;
        ...
        if (matchValue_1 == null) {
            ...
            return [matchValue_1, 0, segmentVs.length - 1];
        }
    }
}
