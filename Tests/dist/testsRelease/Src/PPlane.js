...
export class PPlane extends Record {
    ...
    toString() {
        const pl = this;
        return `Euclid.PPlane(${nl}Origin=${Pnt__get_AsString(pl.Origin)}${nl}  X-axis=${UnitVec__get_AsString(pl.Xaxis)}${nl}  Y-axis=${UnitVec__get_AsString(pl.Yaxis)}${nl}  Z-axis=${UnitVec__get_AsString(pl.Zaxis)})`;
    }
}
...
export function PPlane__get_AsString(pl) {
    return `${nl}Origin=${Pnt__get_AsString(pl.Origin)}${nl}  X-axis=${UnitVec__get_AsString(pl.Xaxis)}${nl}  Y-axis=${UnitVec__get_AsString(pl.Yaxis)}${nl}  Z-axis=${UnitVec__get_AsString(pl.Zaxis)}`;
}
