...
export class BRect extends Record {
    ...
    toString() {
        const r = this;
        return `Euclid.BRect: sizeX=${float(r.MaxX - r.MinX)}| sizeY=${float(r.MaxY - r.MinY)}| at X=${float(r.MinX)}|Y=${float(r.MinY)}`;
    }
}
...
export function BRect__get_AsString(r) {
    return `sizeX=${float(r.MaxX - r.MinX)}| sizeY=${float(r.MaxY - r.MinY)}| at X=${float(r.MinX)}|Y=${float(r.MinY)}`;
}
...
export function BRect__IsTouching_13C371E0(b, a, tol = 1E-06) {
    ...
    if (xOverlap && ((Math.abs(a.MinY - b.MaxY) <= tol) ? true : (Math.abs(b.MinY - a.MaxY) <= tol))) {
        return true;
    }
    else if (xTouch) {
        return yOverlap;
...
export function BRect_expandRel(factor, r) {
    ...
    let center_1;
    const r_1 = r;
    center_1 = Pt_$ctor_7B00E9A0((r_1.MaxX + r_1.MinX) * 0.5, (r_1.MaxY + r_1.MinY) * 0.5);
    const sizeX_1 = ((r_2 = r, r_2.MaxX - r_2.MinX)) * factor;
    const sizeY_1 = ((r_3 = r, r_3.MaxY - r_3.MinY)) * factor;
    if (!(sizeX_1 >= 0)) {
    ...
    return BRect_$ctor_77D16AC0(center_1.X - (sizeX_1 * 0.5), center_1.Y - (sizeY_1 * 0.5), center_1.X + (sizeX_1 * 0.5), center_1.Y + (sizeY_1 * 0.5));
}
...
export function BRect_expandRelXY(factorX, factorY, r) {
    ...
    const r_1 = r;
    center_1 = Pt_$ctor_7B00E9A0((r_1.MaxX + r_1.MinX) * 0.5, (r_1.MaxY + r_1.MinY) * 0.5);
    const sizeX_1 = ((r_2 = r, r_2.MaxX - r_2.MinX)) * factorX;
    const sizeY_1 = ((r_3 = r, r_3.MaxY - r_3.MinY)) * factorY;
    if (!(sizeX_1 >= 0)) {
        fail(`BRect.createFromCenter sizeX is negative: ${sizeX_1}, sizeY is: ${sizeY_1}, center: ${Pt__get_AsString(center_1)}`);
    }
    if (!(sizeY_1 >= 0)) {
