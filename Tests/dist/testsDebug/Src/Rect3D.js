...
import { failRect3DOffsetEdge, failDivide, fail, failTooSmall, fail2, failTooSmall2 } from "./EuclidErrors.js";
...
export class Rect3D extends Record {
    ...
    toString() {
        let v, v_1;
        const r = this;
        const sizeX = float((v = r.Xaxis, Math.sqrt(((v.X * v.X) + (v.Y * v.Y)) + (v.Z * v.Z))));
        const sizeY = float((v_1 = r.Yaxis, Math.sqrt(((v_1.X * v_1.X) + (v_1.Y * v_1.Y)) + (v_1.Z * v_1.Z))));
        const origin = Pnt__get_AsString(r.Origin);
        const xaxis = Vec__get_AsString(r.Xaxis);
        const yaxis = Vec__get_AsString(r.Yaxis);
        return `Euclid.Rect3D ${sizeX} x ${sizeY} (Origin:${origin}| X-ax:${xaxis}|Y-ax:${yaxis})`;
    }
}
...
export function Rect3D_$ctor_6181AF53(origin, axisX, axisY) {
    ...
    let lenX;
    const v = axisX;
    lenX = Math.sqrt(((v.X * v.X) + (v.Y * v.Y)) + (v.Z * v.Z));
    let lenY;
    const v_1 = axisY;
    lenY = Math.sqrt(((v_1.X * v_1.X) + (v_1.Y * v_1.Y)) + (v_1.Z * v_1.Z));
    if (!(lenX > 1E-06)) {
        failTooSmall2("Rect3D() axisX", axisX, axisY);
    }
    if (!(lenY > 1E-06)) {
        failTooSmall2("Rect3D() axisY", axisY, axisX);
    }
    if (Math.abs((a = axisX, (b = axisY, ((a.X * b.X) + (a.Y * b.Y)) + (a.Z * b.Z)))) > ((lenX + lenY) * 1E-09)) {
        fail2("Rect3D(): X-axis and Y-axis are not perpendicular", axisX, axisY);
    }
...
export function Rect3D__get_BBox(r) {
    ...
    const minX = min(min(min(p0.X, p1.X), p2.X), p3.X);
    const minY = min(min(min(p0.Y, p1.Y), p2.Y), p3.Y);
    const minZ = min(min(min(p0.Z, p1.Z), p2.Z), p3.Z);
    const maxX = max(max(max(p0.X, p1.X), p2.X), p3.X);
    const maxY = max(max(max(p0.Y, p1.Y), p2.Y), p3.Y);
    const maxZ = max(max(max(p0.Z, p1.Z), p2.Z), p3.Z);
...
export function Rect3D_createFrom3Points_6180BC13(origin, xPt, yPt) {
    ...
    const a_4 = yu;
    const b_4 = y;
    f_1 = (((a_4.X * b_4.X) + (a_4.Y * b_4.Y)) + (a_4.Z * b_4.Z));
    yr = Vec_$ctor_Z7AD9E565(a_5.X * f_1, a_5.Y * f_1, a_5.Z * f_1);
    return Rect3D_$ctor_6181AF53(origin, x, yr);
}
    ...
    if (!(lx1 > 1E-12) ? true : !(ly1 > 1E-12)) {
        ...
        for (let ix = 0; ix <= (xCount - 1); ix++) {
            ...
            for (let iy = 0; iy <= (yCount - 1); iy++) {
                ...
                setItem(rs, iy, Rect3D_$ctor_6181AF53((p_1 = ((p = o, (v_2 = x_2, Pnt_$ctor_Z7AD9E565(p.X + v_2.X, p.Y + v_2.Y, p.Z + v_2.Z)))), (v_3 = y, Pnt_$ctor_Z7AD9E565(p_1.X + v_3.X, p_1.Y + v_3.Y, p_1.Z + v_3.Z))), vx, vy));
            }
            setItem(rss, ix, rs);
        }
        return rss;
...
export function Rect3D_subDivideMinLength_Z52AC30F2(rect, xMinLen, yMinLen, xGap, yGap) {
    ...
    return Rect3D_subDivide_Z52A8DC92(rect, xCount, yCount, xGap, yGap);
}
...
export function Rect3D_grid_638F50CE(rect, xCount, yCount) {
    ...
    for (let ix = 0; ix <= (xCount - 1); ix++) {
        ...
        for (let iy = 0; iy <= (yCount - 1); iy++) {
            ...
            const f_1 = (ly1 * iy) / yl;
            y = Vec_$ctor_Z7AD9E565(a_1.X * f_1, a_1.Y * f_1, a_1.Z * f_1);
            setItem(rs, iy, (p_1 = ((p = o, (v_2 = x, Pnt_$ctor_Z7AD9E565(p.X + v_2.X, p.Y + v_2.Y, p.Z + v_2.Z)))), (v_3 = y, Pnt_$ctor_Z7AD9E565(p_1.X + v_3.X, p_1.Y + v_3.Y, p_1.Z + v_3.Z))));
        }
        setItem(rss, ix, rs);
    }
    return rss;
...
export function Rect3D_gridMinLength_ZFDD8B12(rect, xMinLen, yMinLen) {
    ...
    return Rect3D_grid_638F50CE(rect, xCount, yCount);
}
