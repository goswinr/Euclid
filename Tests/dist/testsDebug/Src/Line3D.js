...
export class Line3D extends Record {
    ...
    toString() {
        ...
        const ln = this;
        const fromX = float(ln.FromX);
        const fromY = float(ln.FromY);
        const fromZ = float(ln.FromZ);
        const toX = float(ln.ToX);
        const toY = float(ln.ToY);
        const toZ = float(ln.ToZ);
        const length = float((ln_1 = ln, (x = ((ln_2 = ln_1, ln_2.ToX - ln_2.FromX)), (y = ((ln_3 = ln_1, ln_3.ToY - ln_3.FromY)), (z = ((ln_4 = ln_1, ln_4.ToZ - ln_4.FromZ)), Math.sqrt(((x * x) + (y * y)) + (z * z)))))));
        return `Euclid.Line3D from X=${fromX}|Y=${fromY}|Z=${fromZ} to X=${toX}|Y=${toY}|Z=${toZ} Length ${length}`;
    }
}
...
export function Line3D__get_AsString(ln) {
    ...
    const toY = float(ln.ToY);
    const toZ = float(ln.ToZ);
    return `${fromX}, ${fromY}, ${fromZ} to ${toX}, ${toY}, ${toZ}`;
}
