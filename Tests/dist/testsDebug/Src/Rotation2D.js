...
import { failUnit2, failRot } from "./EuclidErrors.js";
...
export class Rotation2D extends Record {
    ...
    toString() {
        const r = this;
        const deg = float(57.29577951308232 * Math.atan2(r.Sin, r.Cos));
        return concat("Euclid.Rotation2D of ", deg, ..."° Degrees.");
    }
}
...
export function Rotation2D_$ctor_7B00E9A0(sin, cos) {
    const sum = (sin * sin) + (cos * cos);
    if ((0.99999 > sum) ? true : (sum > 1.00001)) {
        failRot(sin, cos);
    }
...
export function Rotation2D_createFromVectors_67D9FFC0(a, b) {
    ...
    const a_1 = a;
    const b_1 = b;
    dot = ((a_1.X * b_1.X) + (a_1.Y * b_1.Y));
    let cross;
    const a_2 = a;
    const b_2 = b;
    cross = ((a_2.X * b_2.Y) - (a_2.Y * b_2.X));
    return Rotation2D_$ctor_7B00E9A0(cross, dot);
}
